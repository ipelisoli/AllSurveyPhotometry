...
class getPanstarrs(object):
    def getAllData(RAdeg,Decdeg, rad=1/3600):
        ...
        url="https://catalogs.mast.stsci.edu/api/v0.1/panstarrs/dr2/detection.csv?ra="+str(RAdeg)+"&dec="+str(Decdeg)+"&radius="+str(rad)+"&nDetections.gte=10"
        urlmean="https://catalogs.mast.stsci.edu/api/v0.1/panstarrs/dr2/mean.csv?ra="+str(RAdeg)+"&dec="+str(Decdeg)+"&radius="+str(rad)+"&nDetections.gte=10"

        #https://catalogs.mast.stsci.edu/api/v0.1/panstarrs/dr2/detection.votable?ra=210.802429&dec=54.348750&radius=0.0083333
        try:
            urllib.request.urlretrieve(url, "Panstars.csv")
            #urllib.request.urlretrieve(urlmean, "Panstarsmean.csv")
        except:
            print("No PanSTARRS data found.")
