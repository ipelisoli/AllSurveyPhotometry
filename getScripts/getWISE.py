...
class WISE(object):
    ...
    def saveFilters(RAdeg,Decdeg):
        ...
        try:
            ...
            np.savetxt("WISE_W1.dat", np.array([bjd, w1mag, w1mage]).T)

            try:
                mask2=(table['w2mpro_ep'] > 0)
                ...
                np.savetxt("WISE_W2.dat", np.array([bjd, w2mag, w2mage]).T)
            except:
                print("Could not create W2 light curve.")

            try:
                mask3=(table['w3mpro_ep'] > 0)
                mjd=table['mjd'][mask3]
                bjd=miscAstro.jd_corr(mjd,RAdeg,Decdeg,loc=EarthLocation.of_site("lapalma")).value
                w3mag=table['w3mpro_ep'][mask3]
                w3mage=table['w3sigmpro_ep'][mask3]
                np.savetxt("WISE_W3.dat", np.array([bjd, w3mag, w3mage]).T)
            except:
                print("Could not create W3 light curve.")

            try:
                mask4=(table['w4mpro_ep'] > 0)
                mjd=table['mjd'][mask4]
                bjd=miscAstro.jd_corr(mjd,RAdeg,Decdeg,loc=EarthLocation.of_site("lapalma")).value
                w4mag=table['w4mpro_ep'][mask4]
                w4mage=table['w4sigmpro_ep'][mask4]
                np.savetxt("WISE_W4.dat", np.array([bjd, w4mag, w4mage]).T)
            except:
                print("Could not create W4 light curve.")

            return len(table['mjd'])

        except:
            print("Could not create WISE light curves.\n")
            return None

    def Plot():
        try:
            mjd,mag,mage=np.loadtxt("WISE_W1.dat",unpack=True)
            plt.scatter(mjd,mag, c='b', label="WISE1")
            plt.errorbar(mjd,mag,yerr=mage,ls=" ", c='b')
        except: None

        try:
            mjd,mag,mage=np.loadtxt("WISE_W2.dat",unpack=True)
            plt.scatter(mjd,mag, c='g', label="WISE2")
            plt.errorbar(mjd,mag,yerr=mage,ls=" ", c='g')
        except: None

        try:
            mjd,mag,mage=np.loadtxt("WISE_W3.dat",unpack=True)
            plt.scatter(mjd,mag, c='orange', label="WISE3")
            plt.errorbar(mjd,mag,yerr=mage,ls=" ", c='orange')
        except: None

        try:
            mjd,mag,mage=np.loadtxt("WISE_W4.dat",unpack=True)
