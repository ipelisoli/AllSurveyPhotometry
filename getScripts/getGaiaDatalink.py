...
class GaiaDatalink(object):
    def extract_dl_ind(datalink_dict, key, RVS, figsize = [15,5], fontsize = 12, linewidth = 2, show_legend = True, show_grid = True):
        ...
        if 'time' in dl_out.keys():
            ...
            gbands = ['G', 'RP', 'BP']
            for band in gbands:
                phot_set = dl_out[dl_out['band'] == band]
                mask = ~np.isnan(phot_set['time'])
                phot_set = phot_set[mask]
                np.savetxt("Gaia_"+str(band)+".dat", np.array([phot_set['time'].value+ 2455197.5- 2400000.5, phot_set['mag'].value, 1.086/phot_set['flux_over_error'].value]).T, fmt="%s")
            print("Found epoch photometry.\n")
        if 'wavelength' in dl_out.keys():
            if len(dl_out) == 343:
                title = 'XP Sampled'
                file = "Gaia_XPspec.txt"
                print("Found XP spectra.\n")
            if len(dl_out) == 2401:
                title = 'RVS'
    ...
    def getData(source_id_input):
        ...
        for i in dl_key:
            if "RVS" in i:
                ...
                RVS=False
            #try:
            GaiaDatalink.extract_dl_ind(datalink, i,RVS, figsize=[20,7])
                #print(dl_out)
